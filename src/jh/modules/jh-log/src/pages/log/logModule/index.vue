...
<template>
	<div class="logType routerContent">
		...
		<moduleDialog
			...
			:moduleDialogVisible.sync="moduleDialogVisible"
		/>
	</div>
</template>
...
export default {
	name: 'logType',
	components: {
		moduleDialog: (resolve) => resolve(require('./components/moduleDialog')),
	},
	...
	data() {
		return {
			...
			columnList: [
				...
				{ prop: 'moduleCode', label: '模块编码', align: 'center' },
	...
	methods: {
		getData(type) {
			...
			this.formShow = false;
			this.$api.formPost('/api/log/module/doGetList', params).then((res) => {
		...
		editData(row) {
			if (row) {
				this.moduleDialogConfig.title = '修改模块信息';
				this.moduleDialogConfig.data = JSON.parse(JSON.stringify(row));
			} else {
				this.moduleDialogConfig.title = '添加模块信息';
				this.moduleDialogConfig.data = null;
			}
			this.moduleDialogVisible = true;
		},
		delData(row) {
			let params = { id: row.id };
			this.$api.formPost('/api/log/module/doRmById', params).then((res) => {
