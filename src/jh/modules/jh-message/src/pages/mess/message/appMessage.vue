<template>
	<div class="appMessage routerContent">
		...
		<el-tabs v-model="activeName" @tab-click="handleTabClick">
			...
			<el-tab-pane label="发送成功" name="second"></el-tab-pane>
			<el-tab-pane label="发送失败" name="three"></el-tab-pane>
		</el-tabs>
	...
	methods: {
		getData() {
			...
			if (this.activeName === 'first') {
				...
				this.$api.formPost('/api/msg/app/doGetHisList', params).then((res) => {
					load.close();
		...
		handleDelete() {
			...
			this.$confirm('此操作将永久删除该消息, 是否继续?', '提示', {
				...
				.then(() => {
					let params = {};
					params.ids = ids;
					let url = '/api/msg/app/doRmHisByIds';
					if (this.activeName == 'first') {
		...
		handleRetrySend(row) {
			this.$confirm('此操作将重新发送该消息, 是否继续?', '提示', {
				...
				.then(() => {
					let params = {};
					params.id = row.id;
					let url = '/api/msg/app/doRetryById';
					this.$api.formPost(url, params).then((res) => {
						if (res.retHead.code == '0000000') {
							this.$message({
								message: '操作成功！',
