	...
	methods: {
		getData() {
			...
			if (this.ruleForm.createTime && this.ruleForm.createTime.length == 2) {
				param.fromhangTableCreateTime = this.ruleForm.createTime[0];
			...
			if (this.activeName === 'first') {
				...
				this.formShow = false;
				this.$api.formPost('/api/msg/wx/doGetHisList', params).then((res) => {
					load.close();
		...
		handleDelete() {
			...
			this.$confirm('此操作将永久删除该消息, 是否继续?', '提示', {
				...
				.then(() => {
					...
					let url = '/api/msg/wx/doRmHisByIds';
					if (this.activeName == 'first') {
		...
		handleRetrySend(row) {
			this.$confirm('此操作将重新发送该消息, 是否继续?', '提示', {
				...
				.then(() => {
					let params = {};
					params.id = row.id;
					let url = '/api/msg/wx/doRetryById';
					this.$api.formPost(url, params).then((res) => {
						if (res.retHead.code == '0000000') {
							this.$message({
								message: '操作成功！',
