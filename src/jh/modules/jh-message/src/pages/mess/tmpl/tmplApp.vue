<template>
  <div class="tmplApp routerContent">
  ...
  methods: {
    ...
    handleAdd() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          ...
          this.$api
            .formPost("/api/mess/tmpl/appTemplate/doSave", addFormParams)
            .then(res => {
              if (res.retHead.code == "0000000") {
                this.$message({
                  message: "添加成功！",
    ...
    handleEdit() {
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          this.$api
            .formPost("/api/mess/tmpl/appTemplate/doModById", editFormParams)
            .then(res => {
              if (res.retHead.code == "0000000") {
    ...
    handleDelete(index, row) {
      this.$confirm("此操作将永久删除该项, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$api
            .formPost("/api/mess/tmpl/appTemplate/doRmById", {
              id: row.id
            })
            .then(res => {
              if (res.retHead.code == "0000000") {
    ...
    handleDisplay() {
      ...
      this.$api
        .formPost("/api/mess/tmpl/appTemplate/doDisplayMess", editFormParams)
        .then(res => {
          if (res.retHead.code == "0000000") {
