<template >
  ...
  <div class="cron" :val="value_">
    ...
    <el-input v-model="val" placeholder="请输入内容">
      <i slot="suffix" class="el-input__icon el-icon-success" @click="setCron"></i>
    </el-input>
    </div>
...
<script>
import second from "./cron/second";
import minute from "./cron/minute";
  ...
  methods: {
    ...
    onInit(cron){
      ...
      this.$nextTick(() => {
        ...
        this.dVal=valus[3];
        if(valus.length>=5)
          this.monthVal=valus[4];
        if(valus.length>=6)
          this.weekVal=valus[5];
        else
          this.weekVal='?';
        if(valus.length>=7)
          this.yearVal=valus[6]; 
        else
          this.yearVal='';   
