  ...
  <div :val="value_">
    <el-radio-group v-model="type">
      ...
      <div>
        ...
        <span style="margin-left: 10px; margin-right: 5px;">从</span>
        <el-input-number  @change="type = '3'" :max="12" :min="1" v-model="loop.start"></el-input-number>
        <span style="margin-left: 5px; margin-right: 5px;">开始，每</span>
        <el-input-number  @change="type = '3'" :max="12" :min="1" v-model="loop.end"></el-input-number>
        月执行一次
      </div>  
      <div>
        ...
        <div v-for="i in 2" :key="i" style="margin-left: 10px; line-height: 25px;">
          <el-checkbox v-for="j in 10" v-if="parseInt((i - 1) + '' + (j - 1)) < 13 && !(i === 1 && j === 1)" :key="j" :label="(i - 1) + '' + (j - 1)"></el-checkbox >
        </div>
      </el-checkbox-group >
      </div>
    </el-radio-group>
