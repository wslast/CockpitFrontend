...
-->
<template>
	<div class="seqnum routerContent">
		...
		<infoDialog
			...
			:seqnumDialogConfig="seqnumConfig"
			v-if="seqnumVisible"
			:seqnumDialogVisible.sync="seqnumVisible"
		/>
	</div>
</template>
...
export default {
	...
	methods: {
		editData(row) {
			if (row) {
				this.seqnumConfig.title = '修改序列值';
				this.seqnumConfig.data = JSON.parse(JSON.stringify(row));
			} else {
				this.seqnumConfig.title = '';
				this.seqnumConfig.data = null;
			}
			this.seqnumVisible = true;
		},
		delData(row) {
		...
		getData(type) {
			...
			this.formShow = false;
			this.$api
				.formPost('/api/core/seq/seqnum/doGetList', params)
				.then((res) => {
					load.close();
